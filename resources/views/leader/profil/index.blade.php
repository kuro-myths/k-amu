        ...
        <div class="col-md-4">
            <div class="card text-center">
                <div class="card-body">
                    ...
                    <p class="badge bg-primary">{{ ucfirst(auth()->user()->role) }}</p>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function uploadAvatar(file) {
            ...
            fetch('{{ route("leader.profile.upload-avatar") }}', {
                    method: 'POST',
                    headers: {
        ...
        if (deleteAvatarBtn) {
            deleteAvatarBtn.addEventListener('click', (e) => {
                ...
                if (confirm('Apakah Anda yakin ingin menghapus avatar?')) {
                    fetch('{{ route("leader.profile.delete-avatar") }}', {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('[name="_token"]').value,
        ...
        profileForm.addEventListener('submit', (e) => {
            ...
            fetch('{{ route("leader.profile.update") }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('[name="_token"]').value
                    },
